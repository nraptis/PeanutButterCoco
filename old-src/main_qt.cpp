...
constexpr peanutbutter::ExpansionStrength kDefaultBundleTableStrength =
    peanutbutter::ExpansionStrength::kLow;
constexpr unsigned int kWorkerThreadStackSizeBytes = 128u * 1024u * 1024u;
// constexpr peanutbutter::EncryptionStrength kDefaultBundleEncryptionStrength =
...
peanutbutter::CryptGenerator MakePresetCryptGenerator() {
  return [](const peanutbutter::CryptGeneratorRequest& pRequest,
    ...
    LogCryptGenerationStatus(
        pRequest,
        "[Expansion] Tables disabled. Using rotation mask cipher.");
    ReportCryptGenerationProgress(
        pRequest, peanutbutter::CryptGenerationStage::kExpansion, 1.0);
    ReportCryptGenerationProgress(
        pRequest, peanutbutter::CryptGenerationStage::kLayerCake, 0.0);
    std::unique_ptr<peanutbutter::Crypt> aCrypt =
        peanutbutter::MakeRotationMaskCrypt(pRequest);
    if (aCrypt == nullptr) {
      if (pErrorMessage != nullptr) {
        *pErrorMessage = "failed creating rotation mask crypt.";
      }
      return {};
    }
    ReportCryptGenerationProgress(
        pRequest, peanutbutter::CryptGenerationStage::kLayerCake, 1.0);
    LogCryptGenerationStatus(
        pRequest, "[LayerCake] Rotation mask crypt created.");
    return aCrypt;
  };
}
...
struct TableStrengthOption {
  const char* mLabel = "";
  peanutbutter::ExpansionStrength mStrength = peanutbutter::ExpansionStrength::kHigh;
};

struct RepairBlocksOption {
  const char* mLabel = "";
  std::uint8_t mPercent = 0u;
};
...
const std::array<TableStrengthOption, 3> table_strength_options = {{
    {"Low", peanutbutter::ExpansionStrength::kLow},
    {"Medium", peanutbutter::ExpansionStrength::kMedium},
    {"High", peanutbutter::ExpansionStrength::kHigh},
}};

const std::array<RepairBlocksOption, 5> repair_blocks_options = {{
    {"0%", 0u},
    {"25%", 25u},
    {"50%", 50u},
    {"75%", 75u},
...
struct PBBundleRequest {
  ...
  peanutbutter::ExpansionStrength mTableStrength = kDefaultBundleTableStrength;
  std::uint8_t mRepairBlockPercent = 0u;
  bool mUseEncryption = false;
  bool mSparseMode = false;
};
...
class PBMockEngine final {
 public:
  ...
  PBOperationResult RunBundle(const PBBundleRequest& pRequest,
    ...
    mLogger.LogStatus(std::string("[Bundle][Options] Encryption=") +
                      (pRequest.mUseEncryption ? "on" : "off") +
                      ", Sparse=" + (pRequest.mSparseMode ? "on" : "off") +
                      ", Repair Blocks=" +
                      std::to_string(static_cast<unsigned int>(pRequest.mRepairBlockPercent)) + "%.");
    ...
    aCoreRequest.mExpansionStrength = pRequest.mTableStrength;
    aCoreRequest.mCryptGenerator = mCryptGenerator;

    if (pRequest.mSparseMode) {
      mLogger.LogStatus("[Bundle][Options] Sparse mode UI is enabled; core sparse packing is not implemented yet.");
    }
    if (pRequest.mRepairBlockPercent > 0u) {
      mLogger.LogStatus("[Bundle][Options] Repair block reservation is not implemented yet; continuing without a repair sector.");
    }
  ...
  PBOperationResult RunDecode(const PBDecodeRequest& pRequest,
                              ...
                              const std::shared_ptr<std::atomic_bool>& pCancelRequested) {
    LogBuildConstantsOnce();
    const std::string aModeName = (pIntent == PBDecodeIntent::Repair) ? "Repair" : "Unbundle";
    const std::string aDiscoveryPrefix = (pIntent == PBDecodeIntent::Repair) ? "[Repair][Discovery]" : "[Decode][Discovery]";
    ...
    aCoreRequest.mPassword = pRequest.mPassword;
    aCoreRequest.mUseEncryption = pRequest.mUseEncryption;
    aCoreRequest.mRecoverMode = (pIntent == PBDecodeIntent::Repair);
    aCoreRequest.mCryptGenerator = mCryptGenerator;

    peanutbutter::OperationResult aCoreResult;
    if (pIntent == PBDecodeIntent::Repair) {
...
class PBQtController final : public QObject {
 public:
  ...
  void TriggerRepairFlow(const PBDecodeRequest& pRequest) {
    TriggerFileFlow(
        "Repair",
        ResolveRequestPaths(mFileSystem, pRequest),
        [this](const PBDecodeRequest& pValue) { return mEngine.CheckDecode(pValue, PBDecodeIntent::Repair); },
        [this](const PBDecodeRequest& pValue,
               PBDestinationAction pAction,
               const std::shared_ptr<std::atomic_bool>& pCancelRequested) {
          return mEngine.RunDecode(pValue, PBDecodeIntent::Repair, pAction, pCancelRequested);
        },
        [](const PBDecodeRequest& pValue) { return pValue.mDestinationPath; });
  }
 ...
 private:
  ...
  void StartOperation(const QString& pTitle, const QString& pDetail, tOperation pOperation) {
    ...
    QThread* const aThread = QThread::create(
        [aSelf, aCancelRequested, aOperation = std::move(pOperation)]() mutable {
    ...
    aThread->setStackSize(kWorkerThreadStackSizeBytes);
    QObject::connect(aThread, &QThread::finished, aThread, &QObject::deleteLater);
...
int main(int argc, char* argv[]) {
  ...
  auto* repair_blocks_combo = new QComboBox(&window);
  auto* encryption_strength_combo = new QComboBox(&window);
  auto* table_strength_combo = new QComboBox(&window);
  auto* sparse_mode_checkbox = new QCheckBox("Sparse Mode", &window);
  auto* encrypted_checkbox = new QCheckBox("Encrypted", &window);
  ...
  auto* open_archive_encrypted_checkbox = new QCheckBox("Encrypted", &window);
  auto* unbundle_button = new QPushButton("Unbundle", &window);
  auto* repair_archive_source_edit = new QLineEdit(&window);
  auto* repair_archive_destination_edit = new QLineEdit(&window);
  auto* repair_archive_source_file_button = new QPushButton("File...", &window);
  auto* repair_archive_source_folder_button = new QPushButton("Folder...", &window);
  auto* repair_archive_destination_folder_button = new QPushButton("Folder...", &window);
  auto* repair_archive_password_edit = new QLineEdit(&window);
  auto* repair_archive_encrypted_checkbox = new QCheckBox("Encrypted", &window);
  auto* repair_button = new QPushButton("Repair", &window);
  ...
  auto* repair_tab = new QWidget(workflow_tabs);
  auto* repair_tab_layout = new QVBoxLayout(repair_tab);
  auto* repair_stack_host = new QWidget(repair_tab);
  auto* repair_stack = new QStackedLayout(repair_stack_host);
  auto* repair_content = new QWidget(repair_stack_host);
  auto* repair_layout = new QVBoxLayout(repair_content);
  auto* repair_loading = new QWidget(repair_stack_host);
  auto* repair_loading_layout = new QVBoxLayout(repair_loading);
  auto* repair_loading_title = new QLabel("Working", repair_loading);
  auto* repair_loading_indicator = new QProgressBar(repair_loading);
  auto* repair_loading_detail = new QLabel("Operation is running.", repair_loading);
  auto* repair_loading_cancel_button = new QPushButton("Cancel", repair_loading);
  ...
  auto* repair_separator_1 = makePanelSeparator(repair_tab);
  auto* repair_separator_2 = makePanelSeparator(repair_tab);
  ...
  repair_archive_source_edit->setPlaceholderText("Repair Archive Source");
  repair_archive_destination_edit->setPlaceholderText("Repair Output Folder");
  ...
  for (QLabel* aLabel : {make_archive_loading_title,
                         ...
                         repair_loading_title,
                         repair_loading_detail,
                         compare_loading_title,
                         compare_loading_detail}) {
    aLabel->setAlignment(Qt::AlignHCenter);
  }
  for (QLabel* aLabel : {make_archive_loading_detail,
                         open_archive_loading_detail,
                         repair_loading_detail,
                         compare_loading_detail}) {
  ...
  for (QLineEdit* aEdit : {make_archive_source_edit,
                           ...
                           repair_archive_source_edit,
                           repair_archive_destination_edit,
                           compare_left_edit,
                           compare_right_edit,
                           make_archive_file_prefix_edit,
                           make_archive_password_edit,
                           open_archive_password_edit,
                           repair_archive_password_edit}) {
  ...
  for (QPushButton* aButton : {make_archive_source_folder_button,
                               open_archive_source_folder_button,
                               repair_archive_source_folder_button}) {
  ...
  for (QPushButton* aButton : {make_archive_source_file_button,
                               ...
                               repair_archive_source_file_button}) {
    aButton->setFixedHeight(kElementHeight);
  ...
  for (QPushButton* aButton : {make_archive_destination_folder_button,
                               ...
                               repair_archive_destination_folder_button,
  ...
  for (QPushButton* aButton : {bundle_button, unbundle_button, repair_button, compare_button, clear_logs_button}) {
    aButton->setFixedHeight(kElementHeight);
    aButton->setFixedWidth(kButtonGroupWidth);
    aButton->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
  }
  ...
  repair_button->setStyleSheet(
      "QPushButton {"
      "  background-color: #463316;"
      "  color: #fff9f0;"
      "  border: 1px solid #c68b3f;"
      "  border-radius: 8px;"
      "  padding: 0 12px;"
      "}"
      "QPushButton:hover {"
  ...
  for (QComboBox* aComboBox : {archive_size_combo,
                               repair_blocks_combo,
                               encryption_strength_combo,
                               table_strength_combo}) {
  ...
  for (QCheckBox* aCheckBox : {sparse_mode_checkbox,
                               encrypted_checkbox,
                               open_archive_encrypted_checkbox,
                               repair_archive_encrypted_checkbox}) {
    aCheckBox->setFixedHeight(kElementHeight);
    aCheckBox->setFixedWidth(kButtonGroupWidth);
    aCheckBox->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    aCheckBox->setLayoutDirection(Qt::RightToLeft);
    aCheckBox->setStyleSheet("QCheckBox { spacing: 10px; }");
  }
  ...
  auto* make_archive_modes_host = new QWidget(make_archive_tab);
  auto* make_archive_modes_layout = new QHBoxLayout(make_archive_modes_host);
  auto* make_archive_modes_label = new QLabel("Archive Mode", make_archive_modes_host);
  make_archive_modes_layout->setContentsMargins(0, 0, 0, 0);
  make_archive_modes_layout->setSpacing(8);
  make_archive_modes_label->setMinimumWidth(140);
  make_archive_modes_label->setAlignment(Qt::AlignVCenter | Qt::AlignRight);
  make_archive_modes_layout->addWidget(make_archive_modes_label);
  make_archive_modes_layout->addStretch(1);
  make_archive_modes_layout->addWidget(encrypted_checkbox, 0, Qt::AlignRight);
  make_archive_modes_layout->addWidget(sparse_mode_checkbox, 0, Qt::AlignRight);
  make_archive_modes_layout->addWidget(repair_blocks_combo, 0, Qt::AlignRight);
  make_archive_modes_layout->addWidget(table_strength_combo, 0, Qt::AlignRight);
  ...
  open_archive_footer_layout->addWidget(open_archive_encrypted_checkbox, 0, Qt::AlignRight);
  open_archive_footer_layout->addWidget(unbundle_button, 0, Qt::AlignRight);

  auto* repair_footer_host = new QWidget(repair_tab);
  auto* repair_footer_layout = new QHBoxLayout(repair_footer_host);
  auto* repair_password_label = new QLabel("Password", repair_footer_host);
  repair_footer_layout->setContentsMargins(0, 0, 0, 0);
  repair_footer_layout->setSpacing(8);
  repair_password_label->setMinimumWidth(140);
  repair_password_label->setAlignment(Qt::AlignVCenter | Qt::AlignRight);
  repair_footer_layout->addWidget(repair_password_label);
  repair_footer_layout->addWidget(repair_archive_password_edit, 1);
  repair_footer_layout->addWidget(repair_archive_encrypted_checkbox, 0, Qt::AlignRight);
  repair_footer_layout->addWidget(repair_button, 0, Qt::AlignRight);
  ...
  for (QProgressBar* aIndicator : {make_archive_loading_indicator,
                                   open_archive_loading_indicator,
                                   repair_loading_indicator,
  ...
  for (QPushButton* aButton : {make_archive_loading_cancel_button,
                               open_archive_loading_cancel_button,
                               repair_loading_cancel_button,
                               compare_loading_cancel_button}) {
    aButton->setFixedHeight(kElementHeight);
  ...
  for (auto* aLayout : {make_archive_loading_layout,
                        open_archive_loading_layout,
                        repair_loading_layout,
                        compare_loading_layout}) {
  ...
  repair_loading_layout->addWidget(repair_loading_title, 0, Qt::AlignHCenter);
  repair_loading_layout->addSpacing(16);
  repair_loading_layout->addWidget(repair_loading_indicator, 0, Qt::AlignHCenter);
  repair_loading_layout->addSpacing(16);
  repair_loading_layout->addWidget(repair_loading_detail, 0, Qt::AlignHCenter);
  repair_loading_layout->addStretch(1);
  repair_loading_layout->addWidget(repair_loading_cancel_button, 0, Qt::AlignHCenter | Qt::AlignBottom);
  ...
  repair_archive_source_edit->setText(aDefaultArchivePath);
  repair_archive_destination_edit->setText(aDefaultUnarchivePath);
  ...
  repair_archive_password_edit->setText(aDefaultPassword);
  encrypted_checkbox->setChecked(configBoolValue(aConfigDefaults, "default_encrypted", true));
  open_archive_encrypted_checkbox->setChecked(configBoolValue(aConfigDefaults, "default_open_encrypted", true));
  repair_archive_encrypted_checkbox->setChecked(configBoolValue(aConfigDefaults, "default_repair_encrypted", true));
  sparse_mode_checkbox->setChecked(configBoolValue(aConfigDefaults, "default_sparse_mode", false));
  ...
  int aDefaultRepairBlocksIndex = 0;
  const int aConfiguredRepairPercent =
      std::max(0, std::min(100, static_cast<int>(aConfigDefaults.value(QStringLiteral("default_repair_blocks")).toInt(0))));
  for (std::size_t aIndex = 0; aIndex < repair_blocks_options.size(); ++aIndex) {
    const RepairBlocksOption& aOption = repair_blocks_options[aIndex];
    repair_blocks_combo->addItem(QString::fromUtf8(aOption.mLabel),
                                 QVariant::fromValue(static_cast<int>(aOption.mPercent)));
    if (static_cast<int>(aOption.mPercent) == aConfiguredRepairPercent) {
      aDefaultRepairBlocksIndex = static_cast<int>(aIndex);
    }
  }
  repair_blocks_combo->setCurrentIndex(aDefaultRepairBlocksIndex);
  ...
  int aDefaultTableStrengthIndex = 0;
  const int aConfiguredTableStrength = aConfigDefaults.value(QStringLiteral("default_table_strength")).toInt(
      static_cast<int>(static_cast<std::uint8_t>(kDefaultBundleTableStrength)));
  for (std::size_t aIndex = 0; aIndex < table_strength_options.size(); ++aIndex) {
    const TableStrengthOption& aOption = table_strength_options[aIndex];
    table_strength_combo->addItem(
        QString::fromUtf8(aOption.mLabel),
        QVariant::fromValue(static_cast<int>(static_cast<std::uint8_t>(aOption.mStrength))));
    if (static_cast<int>(static_cast<std::uint8_t>(aOption.mStrength)) == aConfiguredTableStrength) {
      aDefaultTableStrengthIndex = static_cast<int>(aIndex);
    }
  }
  table_strength_combo->setCurrentIndex(aDefaultTableStrengthIndex);

  const auto syncBundleEncryptionWidgets = [&]() {
    const bool aEncrypted = encrypted_checkbox->isChecked();
    encryption_strength_combo->setEnabled(aEncrypted);
    table_strength_combo->setEnabled(aEncrypted);
  };
  syncBundleEncryptionWidgets();
  QObject::connect(encrypted_checkbox, &QCheckBox::toggled, &window, [&](bool) {
    syncBundleEncryptionWidgets();
  });
  ...
  repair_archive_source_edit->installEventFilter(
      new PathDropFilter(aFileSystem, repair_archive_source_edit, PathDropFilter::TargetType::Any));
  repair_archive_destination_edit->installEventFilter(
      new PathDropFilter(aFileSystem, repair_archive_destination_edit, PathDropFilter::TargetType::Folder));
  ...
  make_archive_layout->addWidget(make_archive_modes_host);
  ...
  repair_layout->setContentsMargins(12, 12, 12, 12);
  repair_layout->setSpacing(10);
  repair_layout->addWidget(makeLabeledPathRow(repair_tab,
                                              "Archive Source",
                                              repair_archive_source_edit,
                                              repair_archive_source_file_button,
                                              repair_archive_source_folder_button));
  repair_layout->addWidget(repair_separator_1);
  repair_layout->addWidget(makeLabeledPathRow(repair_tab,
                                              "Output Folder",
                                              repair_archive_destination_edit,
                                              nullptr,
                                              repair_archive_destination_folder_button));
  repair_layout->addWidget(repair_separator_2);
  repair_layout->addWidget(repair_footer_host);
  ...
  repair_stack->setContentsMargins(0, 0, 0, 0);
  repair_stack->addWidget(repair_content);
  repair_stack->addWidget(repair_loading);
  repair_stack->setCurrentWidget(repair_content);
  repair_tab_layout->setContentsMargins(0, 0, 0, 0);
  repair_tab_layout->addWidget(repair_stack_host);
  ...
  workflow_tabs->addTab(repair_tab, "Repair Mode");
  ...
  QObject::connect(repair_archive_source_file_button, &QPushButton::clicked, &window, [&]() {
    const QString aPath = pickFile(&window, "Choose repair source file");
    if (!aPath.isEmpty()) {
      repair_archive_source_edit->setText(aPath);
    }
  });
  QObject::connect(repair_archive_source_folder_button, &QPushButton::clicked, &window, [&]() {
    const QString aPath = pickFolder(&window, "Choose repair source folder");
    if (!aPath.isEmpty()) {
      repair_archive_source_edit->setText(aPath);
    }
  });
  QObject::connect(repair_archive_destination_folder_button, &QPushButton::clicked, &window, [&]() {
    const QString aPath = pickFolder(&window, "Choose repair destination folder");
    if (!aPath.isEmpty()) {
      repair_archive_destination_edit->setText(aPath);
    }
  });
  ...
  QObject::connect(bundle_button, &QPushButton::clicked, &window, [&]() {
    ...
    aRequest.mTableStrength =
        static_cast<peanutbutter::ExpansionStrength>(static_cast<std::uint8_t>(table_strength_combo->currentData().toInt()));
    aRequest.mRepairBlockPercent = static_cast<std::uint8_t>(repair_blocks_combo->currentData().toInt());
    aRequest.mUseEncryption = encrypted_checkbox->isChecked();
    aRequest.mSparseMode = sparse_mode_checkbox->isChecked();
  ...
  QObject::connect(unbundle_button, &QPushButton::clicked, &window, [&]() {
    ...
    aRequest.mUseEncryption = open_archive_encrypted_checkbox->isChecked();
    aEntryPoint.TriggerUnbundleFlow(aRequest);
  });

  QObject::connect(repair_button, &QPushButton::clicked, &window, [&]() {
    beginLogSession("log_text_repair.txt", true);
    PBDecodeRequest aRequest;
    aRequest.mSourcePath = repair_archive_source_edit->text().toStdString();
    aRequest.mDestinationPath = repair_archive_destination_edit->text().toStdString();
    aRequest.mPassword = repair_archive_password_edit->text().toStdString();
    aRequest.mUseEncryption = repair_archive_encrypted_checkbox->isChecked();
    aEntryPoint.TriggerRepairFlow(aRequest);
  });
