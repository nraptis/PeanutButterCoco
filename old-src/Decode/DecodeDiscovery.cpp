...
bool ResolveSelectionExpansionStrength(const DiscoverySelection& pSelection,
                                       ExpansionStrength& pOutStrength,
                                       std::string& pOutErrorMessage) {
  bool aFoundReadableHeader = false;
  for (const ArchiveCandidate& aCandidate : pSelection.mArchives) {
    if (!aCandidate.mHasReadableHeader) {
    ...
    if (!aFoundReadableHeader) {
      pOutStrength = aCandidate.mHeader.mExpansionStrength;
      aFoundReadableHeader = true;
      continue;
    }
    if (aCandidate.mHeader.mExpansionStrength != pOutStrength) {
      pOutErrorMessage =
          "selected archive family has mismatched expansion strengths.";
      return false;
    }
  }
  if (!aFoundReadableHeader) {
    pOutErrorMessage =
        "selected archive family did not expose a readable expansion strength.";
    return false;
  }
  return true;
}
