...
const char* DecodeModeName(bool pRecoverMode) {
  return pRecoverMode ? "Repair" : "Unbundle";
}
...
OperationResult RunDecodeCore(const UnbundleRequest& pRequest,
  ...
  if (pRequest.mUseEncryption) {
    ...
    ExpansionStrength aExpansionStrength = ExpansionStrength::kHigh;
    std::string aExpansionError;
    if (!ResolveSelectionExpansionStrength(
            aSelection, aExpansionStrength, aExpansionError)) {
      return MakeFailure(
          ErrorCode::kArchiveHeader,
          aExpansionError.empty()
              ? "failed resolving archive expansion strength."
              : aExpansionError,
          pLogger);
    }
    CryptGeneratorRequest aCryptRequest;
    aCryptRequest.mEncryptionStrength = aEncryptionStrength;
    aCryptRequest.mExpansionStrength = aExpansionStrength;
    ...
    aCryptRequest.mOperationMode =
        pRecoverMode ? CryptOperationMode::kRecover
                     : CryptOperationMode::kUnbundle;
    ...
    aCryptRequest.mShouldCancel = [pCancelCoordinator]() {
      return pCancelCoordinator != nullptr && pCancelCoordinator->IsCancelRequested();
    };
  ...
  const auto aLogMissingTailFromHeaderCount = [&]() {
    ...
    const std::size_t aMissingArchives = static_cast<std::size_t>(
        aSelection.mHeaderDeclaredMaxIndex - aSelection.mMaxIndex);
    pLogger.LogStatus("[Decode][Discovery] Header-declared archive count exceeds discovered archives by " +
                      std::to_string(aMissingArchives) +
                      "; continuing with the archives that were found.");
  };
  ...
  while (aCursor.mArchiveIndex <= aSelection.mMaxIndex) {
    ...
    if (aBox.mEmpty) {
      ...
      if (aGapBoxesEncountered <= 3u || (aGapBoxesEncountered % 500u) == 0u) {
        pLogger.LogStatus("[Repair] GAP_001 at sequence " +
                          std::to_string(aCursor.mArchiveIndex) +
    ...
    if (aParseError) {
      ...
      if (ResolveStrideJump(aCursor,
        ...
        if (aJumpCount <= 3u || (aJumpCount % 500u) == 0u) {
          pLogger.LogStatus("[Repair] Honoring recovery stride.");
          pLogger.LogStatus("[Repair] Jumping to '" +
  ...
  if (pRecoverMode) {
    const auto aElapsed = std::chrono::duration_cast<std::chrono::seconds>(
        std::chrono::steady_clock::now() - aStart);
    pLogger.LogStatus("[Repair] Reached the end of available archive data before a full terminator was found.");
    pLogger.LogStatus("[Repair] Summary: Successfully recovered " +
                      std::to_string(aParser.FilesWritten()) + " files (" +
                      FormatHumanBytes(aParser.BytesWritten()) + ").");
    pLogger.LogStatus("[Repair] Summary: Failed to recover " +
    ...
    return aReturnFailure(ErrorCode::kRecoverExhausted,
                          "repair mode reached the end of available archive data before a complete logical stream could be recovered.",
                          aCancelUnexpectedEndLog);
  }

  aLogMissingTailFromHeaderCount();
  return aReturnFailure(ErrorCode::kArchiveHeader,
                        "decode reached the end of available archive data before a logical terminator was found.",
                        aCancelUnexpectedEndLog);
}
